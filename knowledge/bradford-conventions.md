# TIE Conventions for AI Agents

This document describes the specific patterns, naming conventions, and architecture
of the NHS Example NHS Trust Trust Integration Engine (TIE).

## 1. Production

- **Main production:** `Trust.Productions.TEST` (in TEST namespace)
...
### Package Structure
```
Trust.                                     Top-level namespace
Trust.Productions.TEST                     Main production class
...
Trust.Interfaces.Canonical.               Canonical normalization layer
...
- **Routers:** "<System> Router" or "<System> Distributor"
  - Example: "Cerner Distributor", "LegacyADT Router", "EPMA Pharmacy Router"
- **Processes:** "<Descriptive Name> Process"
  - Example: "Pharmacy Dose Check Process", "RegionalCareRecord Resource Process"
- **Operations:** "HL7 To <System>" or "HL7 To <System> <MessageType>"
  - Example: "HL7 To WardManagement ADT TCP", "HL7 to LegacyADT", "HL7 to MaternityNetwork"
...
| LegacyPathology | File | PMIP | Pathology orders/results |

## 4. Key Downstream Systems

| System | Protocol | Port/Address |
|--------|----------|-------------|
| A&E (EDSystem) | TCP | 10200 |
| LegacyADT | TCP | 30152 |
| MaternityNetwork | SOAP | HL7Import.asmx |
| SystmOne | TCP | 2357 |
| CRIS | TCP | 7107 |
| ICE New | TCP | 5412-5415 |
| InfectionControlNet | TCP | Various |
| WardManagement | TCP | 10001 |
| EPMA Pharmacy | TCP | Various |

## 5. Routing Architecture

The TIE uses a tiered routing architecture:
...
System Distributor (first-level router)
    â”œâ†’ Domain Router 1 (e.g., "LegacyADT Router")
    â”œâ†’ Domain Router 2 (e.g., "WardManagement Router")
    â”œâ†’ Domain Router 3 (e.g., "EndoscopyReporting Router")
...
### Email Alerting
The `Trust.Operations.NHSMail.NHSMailDispatcher` is the central email operation.
...
Messages from different sources are normalized through `Trust.Interfaces.Canonical`:
...
### RegionalCareRecord/FHIR Integration
...
7. **Follow the Trust package hierarchy** for new interfaces
8. **Test with synthetic HL7 messages** using `EnsLib.HL7.Message` and `SetValueAt()`
9. **Site Trust MRN** is in PID:3(1).1 â€” it's called "TRUST MRN"
