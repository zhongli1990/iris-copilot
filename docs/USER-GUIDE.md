...
- **What should happen** (e.g., "email the safety team", "forward to MaternityNetwork", "log to a file")
- **Any conditions** (e.g., "only if the dose exceeds a safe maximum")

**Example:**
> "When Cerner sends pharmacy orders, check if the dose exceeds the safe maximum from a lookup table. If it does, email pharmsafety@example.nhs.uk with the patient MRN, drug name, dose, and safe max. Also log it to a file for audit."
...
**You can answer in plain English.** If you're unsure about a technical detail, say so — the AI will suggest sensible defaults based on the existing Trust TIE configuration.
...
- English: "Create an ADT router for MaternityNetwork"
- Spanish: "Crea un enrutador ADT para MaternityNetwork"
- Chinese: "为MaternityNetwork创建ADT路由器"
...
AI shows topology: 3 new components added in parallel with existing EPMA route. You confirm.
...
| "Is the MaternityNetwork connection working?" | Specific host health, queue depth, last activity |
...
> "Add a second email recipient — icunurse@example.nhs.uk — but only for Warfarin alerts"
