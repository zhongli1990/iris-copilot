...
|-------|-------|
| **ID** | TC-ENGINE-001 |
| **Steps** | 1. Call `CodeManager.WriteClass("Trust.AIGenerated.Test.HelloWorld", <valid COS source>)` 2. Call `CodeManager.CompileClass("Trust.AIGenerated.Test.HelloWorld")` |
| **Expected** | Class compiled successfully, no errors |
| **Pass Criteria** | `$SYSTEM.OBJ.IsValidClassname("Trust.AIGenerated.Test.HelloWorld")` returns 1 |
...
### TC-AGENT-005: Developer — Follows Trust Conventions

| Field | Value |
|-------|-------|
| **ID** | TC-AGENT-005 |
| **Steps** | Inspect generated class names and structure |
| **Expected** | Classes use `Trust.AIGenerated.*` prefix, follow IRIS naming conventions (no underscores in class names, proper package nesting), include documentation comments |
...
|  | - Email sent to pharmsafety@example.nhs.uk |
|  | - Log line written to DoseAlerts_YYYYMMDD.log |
|  | - Original RDE^O11 also forwarded to EPMA (unchanged) |
| **Pass Criteria** | Email received, log file contains alert line, EPMA receives unmodified order |
...
|  | - Original order forwarded to EPMA |
| **Pass Criteria** | No alert generated, EPMA receives order |
...
|-------|-------|
| **ID** | TC-DEMO-005 |
| **Steps** | 1. Inject RDE^O11 2. Check EPMA Pharmacy Router message trace |
| **Expected** | EPMA receives the original, unmodified RDE^O11 exactly as before the dose-check integration existed |
| **Pass Criteria** | Message trace shows original message in EPMA router, no modifications |
...
| **Pass Criteria** | Exactly 50 log entries, 50 emails, 100 messages forwarded to EPMA |
...
| Namespace | Trust TIE namespace with `Trust.Productions.TEST` loaded |
