 ...
 *
 * Optional env:
 *   TrustDGE_URL=http://localhost:3100
...
const TrustDGE_URL = process.env.TrustDGE_URL || 'http://localhost:3100';
...
async function callChat(message: string): Promise<Json> {
  return postJson(`${TrustDGE_URL}/api/chat`, {
  ...
  const tests: TestCase[] = [
    {
      ...
      name: 'Bridge health',
      run: async () => {
        const health = await getJson(`${TrustDGE_URL}/api/health`);
    ...
    },
    {
      ...
      run: async () => {
        const rsp = await postJson(`${TrustDGE_URL}/api/iris/operate`, {
          namespace: NAMESPACE,
          op: 'query',
    ...
    },
    {
      ...
      run: async () => {
        const res = await fetch(`${TrustDGE_URL}/api/iris/operate`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            namespace: NAMESPACE,
